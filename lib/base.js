...
class BaseClient {
  ...
  async sendTransaction(contractAddress, signer, methodABI, args, options) {
	...
	//To use the Cancun hardfork, where gas=0 is not allowed, uncomment the 2 lines below
    //const latestBlock = await this.web3.eth.getBlock('latest');
    //const baseFeePerGas = latestBlock.baseFeePerGas || this.defaultGas; // Usar baseFeePerGas ou valor padrão se nao estiver disponivel
    ...
    const params = {
      ...
      to: contractAddress,
	  //To use the Cancun hardfork, where gas=0 is not allowed, uncomment the 2 lines below
	  //maxPriorityFeePerGas: '0', // Priority fee zero
      //maxFeePerGas: baseFeePerGas, // Use a baseFeePerGas
	  gasPrice: 0,
      value: '0x0',
      gas: options.gas,
    };
    ...
    //Additional logging
    console.log(`Sending transaction from account: ${signer}`);
    console.log(`Transaction details:`, params);
...
function locateFunctionOrEvent(abi, functionName) {
  ...
  //Now using the full ABI JSON, so we must access the 'abi' key within it.
  const actualAbi = abi.abi ? abi.abi : abi;
  const func = actualAbi.filter((e) => {
