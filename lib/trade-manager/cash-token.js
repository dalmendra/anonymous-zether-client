...
async mint(receiver, amount, isERC20) {
  logger.info(`Minting ${isERC20 ? 'ERC20' : 'ERC1155'} tokens ...`);
  logger.info(`Receiver: ${receiver}, Amount: ${amount}`);

  let func, args;
  if (isERC20) {
    func = locateFunctionOrEvent(erc20ABI, 'mint');
    args = [receiver, amount];
  } else {
    func = locateFunctionOrEvent(erc1155ABI, 'mint');
    args = [receiver, amount];
  }

  try {
    const receipt = await this.sendTransaction(isERC20 ? this.erc20 : this.erc1155, this.authority.address, func, args, { isAdminSigner: true });
    logger.info(`Successfully minted ${amount} tokens to ${receiver} in token contract ${isERC20 ? this.erc20 : this.erc1155} (transactionHash: ${receipt.transactionHash})`);
  ...
  //
  async enableAccount(ethAddress, isERC20) {
    logger.info(`Enabling account ${ethAddress}`);
    const func = locateFunctionOrEvent(isERC20 ? erc20ABI : erc1155ABI, 'enableAccount');
